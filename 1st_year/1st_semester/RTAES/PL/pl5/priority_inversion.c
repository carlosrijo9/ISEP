...
/*  Carlos Rijo 1101626                                             */
/*                                                                  */
/*  Priority Inversion - is a situation where a high-priority task  */
/*  is blocked by a lower-priority task.                            */
...
void *periodic_task(void *arg)
{
    ...
    for (job = 1; job <= number_jobs; job++)
    {
        ...
        case 3:
            ...
            if ((TimeNow.tv_sec - 0.800) > Timefirstt2.tv_sec)
            {
                printf("Deadline for task HIGH PRIO Missed \n\n\n");
                break;
            }
            break;
        ...
        case 2:
            ...
            clock_gettime(CLOCK_MONOTONIC, &TimeNow);
            if ((TimeNow.tv_sec - 0.800) > Timefirstt1.tv_sec)
            {
                printf("Deadline for task MEDIUM PRIO Missed \n\n\n");
                break;
            }
            break;
        ...
        case 1:
            ...
            pthread_mutex_unlock(&mutex);
            clock_gettime(CLOCK_MONOTONIC, &TimeNow);
            if ((TimeNow.tv_sec - 0.800) > Timefirstt3.tv_sec)
            {
                printf("Deadline for task LOW PRIO Missed \n\n\n");
                break;
            }
            break;
...
int main()
{
    ...
    targs[0].period.tv_nsec = 300000000; /* 300 ms */
    ...
    targs[1].period.tv_nsec = 900000000; /* 900 ms */
    ...
    targs[2].period.tv_nsec = 800000000; /* 800 ms */
