...
// Define Pins
#define LIGHTS1_PIN 23
#define LIGHTS2_PIN 22
...
void callback(char *topic, byte *message, unsigned int length) {
  ...
  if (String(topic) == "lights/control") {
    intensity = messageLights.toInt();
    analogWrite(LIGHTS1_PIN, 255 * intensity / 100);
    analogWrite(LIGHTS2_PIN, 255 * intensity / 100);
  }
}
...
void loop() {
  ...
  int errorCode = 0;
  char lightsErrorString[8];

  dtostrf(errorCode, 1, 2, lightsErrorString);
  client.publish("lights/error", lightsErrorString);
}
