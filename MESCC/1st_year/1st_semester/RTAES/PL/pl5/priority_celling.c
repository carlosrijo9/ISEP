...
int set_task_affinity(int core)
{
    ...
    retval = pthread_setaffinity_np(pthread_self(), sizeof(cores_mask),
                                    &cores_mask);
    return retval; /* 0 if success.*/
}

struct timespec next_arrival(struct timespec previous, struct timespec
                                                           period)
{
    struct timespec next;
    next.tv_sec = previous.tv_sec + period.tv_sec;
    /* Seconds */
...
int main()
{
    ...
    /* Set the time instant when tasks will arrive ( in less than 2
    seconds ).*/
