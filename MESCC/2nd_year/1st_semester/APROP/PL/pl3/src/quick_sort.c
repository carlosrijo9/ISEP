...
void sortTaskGroup(int num_threads, int array[], int low, int high);
void sortParallelLoop(int num_threads, int array[], int low, int high);
...
int main(int argc, char *argv[])
{
	...
	begin = omp_get_wtime();
	sortTaskGroup(num_threads, array, 0, ARR_LEN - 1);
	end = omp_get_wtime();
	...
	// printf("Parallel Loop Sorting...");
	// begin = omp_get_wtime();
	// sortParallelLoop(num_threads, array, 0, ARR_LEN - 1);
	// end = omp_get_wtime();
	// double par_loop_time = (double)(end - begin);
	// printf(" done!\n");
	// // then invoke the following code to validate your version
	// assert(expected, array, ARR_LEN);
