...
#define ARR_LEN 50000000 // The length of the array, increase to have more execution time
#define LOW_LIMIT ARR_LEN / 1000
#define MAX_PRINT 30
...
void quick_sort_par_and_seq(int array[], int low, int high, int num_threads);
...
int main(int argc, char *argv[])
{
    ...
    /**************** Parallel Sorting (with limits) ***************************************/
	printf("Parallel and Sequencial Sorting...");
	begin = omp_get_wtime();
	quick_sort_par_and_seq(array, 0, ARR_LEN - 1, num_threads);
	end = omp_get_wtime();
	...
	printf("Parallel time (with limit %d): %fs\n", LOW_LIMIT, par_seq_time);
	return 0;
}
...
void quick_sort_par(int array[], int low, int high, int num_threads)
{
	if (low < high)
	{
		...
		#pragma omp parallel sections default(shared) firstprivate(m) num_threads(num_threads)
		{
			omp_set_num_threads(num_threads);
			#pragma omp section
			{
				// Sort the left subarray
				// quick_sort_par(array, low, m - 1, num_threads / 2);
				quick_sort_par(array, low, m - 1, num_threads/2);
			}
			#pragma omp section
			{
				// Sort the right subarray
				// quick_sort_par(array, m + 1, high, num_threads / 2);
				quick_sort_par(array, m + 1, high, num_threads/2);
			}
		}
	}
...
void quick_sort_par_and_seq(int array[], int low, int high, int num_threads)
{
	if (low < high)
	{
		...
		{
			// Use sequential sorting for small subarrays
			quick_sort_seq(array, low, high);
		}
		else
		{
            // Otherwise, parallelize the sorting of left and right subarrays
			#pragma omp parallel sections default(shared) firstprivate(m) num_threads(num_threads)
			{
				omp_set_num_threads(num_threads);
				#pragma omp section
				{
					// Sort the left subarray
					// quick_sort_par(array, low, m - 1, num_threads / 2);
					quick_sort_par_and_seq(array, low, m - 1, num_threads);
				}
				#pragma omp section
				{
					// Sort the right subarray
					// quick_sort_par(array, m + 1, high, num_threads / 2);
					quick_sort_par_and_seq(array, m + 1, high, num_threads);
				}
			}
		}
	}
...
/************ Util. Functions ******************************************************************************************************************/
/* This function takes last element as index, places the index element atits correct position in sorted array, and places all smaller
(smaller than the index) to left of the index and all greater elements to right of the index */
int split(int array[], int low, int high)
{
...
/************ Print an array (with a limit on the number of elements) ******/
void print_array(int array[], int size, const char *version)
{
