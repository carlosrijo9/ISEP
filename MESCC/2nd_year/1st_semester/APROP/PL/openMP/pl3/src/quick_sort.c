...
void sortTaskGroup(int array[], int low, int high, int num_threads);
...
int main(int argc, char *argv[])
{
	...
	printf("Parallel Task Group Sorting...");
	begin = omp_get_wtime();
	sortTaskGroup(array, 0, ARR_LEN - 1, num_threads);
	end = omp_get_wtime();
	double task_group_time = (double)(end - begin);
	printf(" done!\n");
	// then invoke the following code to validate your version
	assert(expected, array, ARR_LEN);
	...
	printf("Parallel Task Group time: %fs\n", task_group_time);
	return 0;
}
